...
void callEngineStep1(
    Unit callData, String alertType, String txCode, String batteryIndicator) {
  ...
  Alert newAlert = Alert(alertType, callData.zoneId, callData.name, null, dt,
      txCode, callData.txid, const Uuid().v4(), batteryLvl);
...
void callFilter(Unit callData, Alert newAlert) {
  ...
  if (newAlert.alertType == "3") {
    ...
    liveCallStack[callData.txid] = newAlert;
    CallData cr = CallData(
        null,
        oldAlert.callPointId,
        oldAlert.alertType,
        oldAlert.alertTime,
        now,
        now.difference(oldAlert.alertTime).inSeconds,
        newAlert.alertType,
        "carer",
        oldAlert.alertUuid,
        oldAlert.alertTime.day,
        oldAlert.alertTime.month,
        oldAlert.alertTime.year,
        "serverRef",
        oldAlert.zone,
        oldAlert.room,
        newAlert.alertUuid);
        ...
        case "4":
          {
            if (newAlert.alertType != "2" &&
                newAlert.alertType != "6" &&
                newAlert.alertType != "7" &&
                newAlert.alertType != "8") {
              liveCallStack[callData.txid] = newAlert;
              oldAlertReplaced = true;
            }
          }
